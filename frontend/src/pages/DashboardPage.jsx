...
export const DashboardPage = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      {/* Hero Banner */}
      <div className="relative rounded-3xl overflow-hidden glass-panel border border-white/[0.08] p-8 sm:p-12 mb-10 shadow-2xl">
        ...
        <div className="relative z-10 max-w-2xl">
          <div className="inline-flex items-center gap-2 px-3.5 py-1 rounded-full bg-violet-500/10 border border-violet-500/25 text-violet-300 text-xs font-mono mb-4 shadow-inner">
            <Zap className="w-3.5 h-3.5 text-amber-400 animate-pulse" />
            <span>PostgreSQL Row Locking & WebSockets</span>
          </div>

          <h1 className="text-3xl sm:text-5xl font-extrabold tracking-tight bg-gradient-to-br from-white via-zinc-100 to-zinc-400 bg-clip-text text-transparent mb-4 leading-tight">
          ...
          <div className="flex flex-wrap items-center gap-3">
            {isAuthenticated && (user?.role === 'auctioneer' || user?.role === 'admin') ? (
              <Link
                to="/create"
                className="btn-primary inline-flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-semibold shadow-lg shadow-violet-600/30 transition-all"
              >
            ...
            <button
              ...
              className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-medium text-zinc-300 bg-[#10121C] hover:bg-[#181B28] border border-white/[0.08] transition-all cursor-pointer"
            >
        ...
        <div className="mt-8 pt-6 border-t border-white/[0.08] grid grid-cols-2 sm:grid-cols-3 gap-4 max-w-lg">
          <div>
            <div className="text-2xl font-extrabold text-white font-mono">{auctions.length}</div>
            <div className="text-xs text-zinc-400">Total Auctions</div>
          </div>
          <div>
          ...
          <div className="col-span-2 sm:col-span-1">
            ...
            <div className="text-xs text-zinc-400">ACID Guaranteed</div>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-4 mb-8">
        ...
        <div className="relative w-full md:w-80">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2 rounded-xl text-sm bg-[#0B0D14] border border-white/[0.08] focus:border-violet-500 focus:outline-none text-zinc-100 placeholder:text-zinc-500 transition-colors"
          />
        </div>

        {/* Status Filters */}
        <div className="flex items-center gap-1.5 p-1 rounded-xl bg-[#0B0D14] border border-white/[0.08] w-full md:w-auto overflow-x-auto">
          {[
            { id: 'ALL', label: 'All' },
      ...
      {error ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {[1, 2, 3, 4, 5, 6].map((n) => (
            <div
              key={n}
              className="h-64 rounded-2xl bg-[#0B0C13] border border-white/[0.06] animate-pulse p-5 flex flex-col justify-between"
            >
              ...
              <div className="h-10 bg-white/[0.06] rounded w-full" />
            </div>
          ))}
        </div>
      ) : filteredAuctions.length === 0 ? (
        <div className="glass-card border border-white/[0.08] p-12 rounded-3xl text-center">
          <div className="w-12 h-12 rounded-2xl bg-violet-950/40 border border-violet-500/30 flex items-center justify-center mx-auto mb-4 text-violet-400 shadow-inner">
            <Flame className="w-6 h-6 text-amber-400" />
          </div>
          <h3 className="text-lg font-bold text-zinc-100 mb-1">No Auctions Found</h3>
          ...
          {isAuthenticated && (user?.role === 'auctioneer' || user?.role === 'admin') && (
            <Link
              to="/create"
              className="btn-primary inline-flex items-center gap-2 px-5 py-2.5 rounded-xl text-xs font-semibold shadow-lg shadow-violet-600/30"
            >
              <Plus className="w-4 h-4" />
